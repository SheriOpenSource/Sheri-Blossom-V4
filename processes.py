...
class Bot(AutoShardedBot):
    ...
    async def on_connect(self):
        self.log.info(
            "[Shard Manager] - A shard has successfully connected to discord."
        )

    async def on_disconnect(self):
        self.log.info(
            "[Shard Manager] - A shard has been disconnected from discord. Internet?"
        )
    ...
    async def on_resume(self):
        self.log.info(
            "[Shard Maintainer] - A shard has resumed its connection to discord. Internet? Crash? Maybe?"
        )
    ...
    def error_embed(error):
        ...
        embed.set_footer(
            text="Sheri Blossom, Version: v4.3",
